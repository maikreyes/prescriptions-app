...
export default function PatientPrescriptionDetailPage() {
  ...
  const handleConsume = async () => {
    ...
    setIsConsuming(true);
    try {
      await api.post(`/prescriptions/${params.id}/consume`);
  ...
  const handleDownloadPdf = async () => {
    try {
      const response = await api.get(`/prescriptions/${params.id}/pdf`, {
        responseType: 'blob',
      });
      ...
      link.href = url;
      link.setAttribute('download', `prescription-${prescription?.code}.pdf`);
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              {prescription.status === 'pending' && (
                <Button onClick={handleConsume} isLoading={isConsuming}>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  {prescription.author.speciality && (
                    <p className="text-sm text-gray-500">{prescription.author.speciality}</p>
                  )}
                </div>
